...
def PLAY(s, recu):
    ...
    currentPosition, finalPosition = Move(
        pawnPos, current, random.choice(["forward", "Rdiagonal", "Ldiagonal"])
    )
...
def Move(JEF_towerPosition: list, JEF_currentInStateJson: int, play: str):
    ...
    if JEF_currentInStateJson == 0:
        if play == "forward":
            finalPosition = [
                currentPosition[0] - random.randint(0, currentPosition[0]),
                currentPosition[1],
            ]
        if play == "Rdiagonal":
            finalPosition = [
                currentPosition[0] - random.randint(0, currentPosition[0]),
                currentPosition[1] + random.randint(0, currentPosition[1]),
            ]
        if play == "Ldiagonal":
            finalPosition = [
                currentPosition[0] - random.randint(0, currentPosition[0]),
        ...
        if (
            ...
            or currentPosition[1] == finalPosition[1]
        ):
        ...
        if play == "forward":
            finalPosition = [
                currentPosition[0] + random.randint(0, 7 - currentPosition[0]),
                currentPosition[1],
            ]
        if play == "Rdiagonal":
            finalPosition = [
                currentPosition[0] + random.randint(0, 7 - currentPosition[0]),
                currentPosition[1] - random.randint(0, 7 - currentPosition[1]),
            ]
        if play == "Ldiagonal":
            finalPosition = [
                currentPosition[0] + random.randint(0, 7 - currentPosition[0]),
...
def Sendmove(ls, currentPosition, finalPos, name):
    fun_message = [
        f"{name} did a bold move !",
        f"{name} just subscribed to my OnlyFans !",
        f"domain expansion: Nah, I'd win ",
        f"BOT LOBBY",
        f"gg ez",
        f"pickle",
        f"what is {name} even doing -_-",
        f"when is the competition starting ?",
        f"You tried at least",
        f"BOMBACLAT !",
        f"Waiter ! waiter ! more {name}'s bad moves !",
        f"Am I real ?",
        f"{name} CPU is burning",
        f"SMASH, next question",
        f"{name} just did a 6 7 !",
        f"No Bitches?",
        f"{name} plays LoL everyday",
        f"+1 for the funny message ? ;)",
        f"+1 for the funny message ? ;)",
        f"+1 for the funny message ? ;)",
        f"Are we done ?",
        f"simply better",
        f"YOU NOOB",
        f"Trust me, I'm an engineer",
        f"Trust me bro",
        f"go play the tutorial",
